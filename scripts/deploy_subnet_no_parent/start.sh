...
bootstrap_output=$(cargo make --makefile infra/fendermint/Makefile.toml \
    ...
    -e S3_HOST_PORT="${S3_HOST_PORTS[0]}" \
...
do
  cargo make --makefile infra/fendermint/Makefile.toml \
      ...
      -e S3_HOST_PORT="${S3_HOST_PORTS[i]}" \
...
S3 API:
http://localhost:${S3_HOST_PORTS[0]}
http://localhost:${S3_HOST_PORTS[1]}
http://localhost:${S3_HOST_PORTS[2]}
