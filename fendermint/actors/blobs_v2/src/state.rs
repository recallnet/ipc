...
use fendermint_actor_blobs_v2_shared::params::GetStatsReturn;
use fendermint_actor_blobs_v2_shared::state::{
...
impl State {
    ...
    pub fn update_gas_allowance<BS: Blockstore>(
        ...
        let mut origin_approval = if let Some(sponsor) = sponsor {
            let approval =
                account
                    .approvals_to
                    .hamt(store)?
        ...
        if let Some(delegation) = delegation {
            ...
            origin_account.approvals_from.save_tracked(
                origin_account
                    .approvals_from
                    .hamt(store)?
                    .set_and_flush_tracked(&addr, origin_approval)?,
            );
...
mod tests {
    ...
    use fendermint_actor_blobs_v2_testing::{
