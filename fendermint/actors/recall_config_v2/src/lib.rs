...
use fendermint_actor_recall_config_v2_shared::{
...
pub const ACTOR_NAME: &str = "recall_config_v2";
...
impl Actor {
    ...
    fn set_config(rt: &impl Runtime, params: SetConfigParams) -> Result<(), ActorError> {
        ...
        if params.new_config == 0 {
            return Err(actor_error!(
                illegal_argument,
                "new config must be positive"
            ));
        }
...
mod tests {
    ...
    use fendermint_actor_recall_config_v2_shared::{RecallConfig, RECALL_CONFIG_ACTOR_V2_ID, RECALL_CONFIG_ACTOR_ID};
