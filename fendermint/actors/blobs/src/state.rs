    ...
    pub fn add_blob<BS: Blockstore>(
        ...
        let (sub, blob) = if let Some(mut blob) = blobs.get(&params.hash)? {
            ...
            let sub = if let Some(mut group) = subscribers.get(&caller.subscriber_address())? {
                ...
                new_account_capacity = params.size;
...
mod tests {
    ...
    fn test_finalize_blob_resolved() {
        ...
        // Set to status pending
        let res = state.set_blob_pending(&store, caller, hash, SubscriptionId::default(), source);
        assert!(res.is_ok());
        ...
        for tc in test_cases {
            ...
            for (i, ttl) in blobs_ttls.iter().enumerate() {
                ...
                state
                    ...
                    .unwrap();
                state
                    .set_blob_pending(&store, caller, hash, id.clone(), source)
                    .unwrap();
                state
                    .finalize_blob(
                        &store,
    ...
    fn test_adjust_blob_ttls_for_multiple_accounts() {
        ...
        for i in 0..3 {
            ...
            state
                ...
                .unwrap();
            state
                .set_blob_pending(&store, address1, hash, id.clone(), source)
                .unwrap();
            state
                .finalize_blob(
                    &store,
        ...
        for i in 0..3 {
            ...
            state
                ...
                .unwrap();
            state
                .set_blob_pending(&store, address2, hash, id.clone(), source)
                .unwrap();
            state
                .finalize_blob(
                    &store,
