...
impl EventBuilder {
    ...
    /// Adds a non-indexed parameter to the event.
    pub fn param<T: EvmEventable>(mut self, value: T) -> Self {
        self.param_mut(value, false);
        self
    }

    /// Adds an indexed parameter to the event.
    pub fn param_indexed<T: EvmEventable>(mut self, value: T) -> Self {
        self.param_mut(value, true);
        self
    }

    /// Adds a parameter to the event, returning a builder reference.
    pub fn param_mut<T: EvmEventable>(&mut self, value: T, index: bool) -> &Self {
...
fn to_evm_word(data: &Vec<u8>, fixed_size: bool) -> Result<Vec<u8>, ActorError> {
    let len = data.len();
    match len.cmp(&32) {
        Ordering::Greater => {
            let mut hasher = Keccak256::default();
...
fn to_evm_data(entries: Vec<&Entry>, num_topics: usize) -> Result<Vec<u8>, ActorError> {
    ...
    for entry in entries {
        ...
        if entry.evm_type.fixed_size() {
            ...
            head.extend(to_evm_word(&offset.to_vec(), true)?);
            // Encoded length is added to the tail
            let length = (encoded.len() as u64).to_be_bytes();
            tail.extend(to_evm_word(&length.to_vec(), true)?);
