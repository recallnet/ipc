...
use recall_fendermint_actor_activity_tracker::types::FullActivityRollup;
use recall_fendermint_crypto::PublicKey;
use recall_fendermint_vm_actor_interface::activity::ACTIVITY_TRACKER_ACTOR_ADDR;
...
impl<DB: Blockstore + Clone + 'static> ValidatorActivityTracker for ActorActivityTracker<'_, DB> {
    fn record_block_committed(&mut self, validator: PublicKey) -> anyhow::Result<()> {
        ...
        let msg = FvmMessage {
            ...
            method_num: recall_fendermint_actor_activity_tracker::Method::RecordBlockCommitted
                as u64,
    ...
    fn commit_activity(&mut self) -> anyhow::Result<FullActivity> {
        let msg = FvmMessage {
            ...
            sequence: 0,                // irrelevant
            gas_limit: i64::MAX as u64, // exclude this from gas restriction
            method_num: recall_fendermint_actor_activity_tracker::Method::CommitActivity as u64,
