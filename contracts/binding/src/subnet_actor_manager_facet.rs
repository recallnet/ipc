...
pub mod subnet_actor_manager_facet {
    ...
    impl<M: ::ethers::providers::Middleware> SubnetActorManagerFacet<M> {
        ...
        ///Calls the contract's `join` (0x1ddee554) function
        pub fn join(
            &self,
            ...
            self.0
                .method_hash([29, 222, 229, 84], (public_key, amount, storage))
                .expect("method not found (this should never happen)")
        }
    ...
    ///Container type for all input parameters for the `join` function with signature `join(bytes,uint256,uint256)` and selector `0x1ddee554`
    #[derive(
        Clone,
        ::ethers::contract::EthCall,
        ::ethers::contract::EthDisplay,
